...
const _buildTxV0 = async ({
  ...
  if (additionalSigners) {
    additionalSigners
      .filter((s): s is Signer | PartialSigner => s !== undefined)
      .forEach((signer) => {
        if ('publicKey' in signer && 'partialSign' in signer) {
          tx.partialSign(signer.publicKey, signer);
        } else {
          throw new Error('Invalid additional signer');
        }
      });
  }
  return { tx, lastValidBlockHeight };
