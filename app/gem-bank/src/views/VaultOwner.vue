  ...
  setup() {
    ...
    const moveNFTsOnChain = async () => {
      for (const nft of toVaultNFTs.value) {
        ...
        const publicKey = new PublicKey(nft.mint);
        const nftpublicKey = new PublicKey(nft.pubkey!);


        await depositGem(publicKey, creator, nftpublicKey);
      }
      for (const nft of toWalletNFTs.value) {
        const publicKey = new PublicKey(nft.mint);

        await withdrawGem(publicKey);
      }
